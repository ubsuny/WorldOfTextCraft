...
//
// The configuration files will all support comments in the form of an
// exclamation point ("!"). The configuration files will have fields separated
// by a semicolon (";")
//
// The PC configuration syntax is as follows (omit the < >, they are there for
// notation here only) <name>;<type>;<attack power>;<defense power>;<heal
// power>;
...
//  <name> <type> <attack power> <defense power> <heal power> <multi attack
//  power>
// Example:
// Shemp;Boss;20;0;0;5;
//
//
// There is then a syntax for the battle itself, called a "script". It will
// consist of a set of actions your team will apply per turn.
// <name>;<action>;<target>
...
//  Moe;defend;Shemp;            <<--- Moe switches Shemp's target to be Moe,
//  defends. Larry;heal;Moe;              <<--- Larry heals moe
//
// There is also a command-line interface that will cycle through your players
// and ask for actions prompted from "std::cin" like: Action for Curly?
...
//
//
// The Boss "script" will also be similarly defined, but it will contain a
// description and the boss will attack whoever the target is. The target of the
// Boss is initially set to the first person in your configuration file. For
// instance, Shemp may attack the "first" person on your list (above, this is
// Moe), and then attack everyone on your list. This will happen once per turn.
// Even in the command-line interface, the same sequence is executed once per
// turn. If the "defend" method is called, however, the target of
//
// Shemp;attack;target           <<---- Shemp will attack the first person on
// your list Shemp;attack;all              <<---- Shemp will attack your entire
// party with "multi_attack"
//
// Suppose that your Warrior Moe casts "defend" against Shemp, then Shemp's
// target will be Moe.
//
// --------------------------------------
...
class Battle {
public:
  // Create a "typedef" that sets an alias of one type to another to save
  // typing.
  typedef std::vector<std::shared_ptr<Character>> CharacterVector;

  Battle() : turn_(0), description_(""), user_turns_(false), scripted_(false) {}
  ~Battle() {}
  ...
  void setUserTurns(bool user_turns = true) { user_turns_ = user_turns; }
  bool user_turns() const { return user_turns_; }
...
private:
  ...
  bool user_turns_;          // Input turn-by-turn user input? By default, no.
  CharacterVector pcs_;           // Player characters
  CharacterVector npcs_;          // Non-player characters
  ...
  CharacterVector::iterator find_character(std::string s);

  // This will enumerate the possible actions that can be taken
  enum ActionType { ATTACK = 0, HEAL, DEFEND, MULTIATTACK, N_ACTIONS };

  // A private "struct" (i.e. a class with all public members) to store the
  // source and action type
  struct QuickAction {
    std::shared_ptr<Character> source;
    ActionType action;
    QuickAction(std::shared_ptr<Character> s = 0, ActionType a = N_ACTIONS)
        : source(s), action(a) {}
  };
  std::vector<QuickAction>
      actions_; // List of scripted actions to perform per turn.
