...
bool Battle::readPCConfiguration(std::string filename) {
  ...
  if (pcs_.size() > 0) {
    std::cout << "Configuration already read... skipping reading from "
              << filename << std::endl;
    return false;
  } else {
    std::ifstream in(filename);
    if (!in.is_open()) {
      std::cerr << "[Battle::readPCConfiguration] File not found: " << filename << std::endl;
      exit(1);
    }
    ...
    while (!in.eof()) {
      ...
      if ((line.size() > 0 && line[0] == '!') || line == "") {
        ...
        // First read the type from the first token, and create the appropriate
        // class (Warrior, Priest, Rogue). Then read in the stats from the rest
        // of the line.
        std::stringstream tokens(line);
        std::string character_type;
        std::getline(tokens, character_type, ';');

        std::shared_ptr<Character> character;
        if (character_type == "Warrior") {
          character = std::shared_ptr<Character>(new Warrior());
        } else if (character_type == "Priest") {
          character = std::shared_ptr<Character>(new Priest());
        } else if (character_type == "Rogue") {
          character = std::shared_ptr<Character>(new Rogue());
        } else {
          std::cout << "Character Type not recognized: " << line << ", skipping"
                    << std::endl;
          continue;
        }
...
bool Battle::readNPCConfiguration(std::string filename) {
  if (npcs_.size() > 0) {
    ...
    std::ifstream in(filename);
    if (!in.is_open()) {
      std::cerr << "[Battle::readNPCConfiguration] File not found: " << filename << std::endl;
      exit(1);
    }
    while (!in.eof()) {
      std::string line;
      ...
      if ((line.size() > 0 && line[0] == '!') || line == "") {
        ...
        if (description_ == "") {
          ...
          std::shared_ptr<Character> boss(new Boss());
          boss->input(line);
...
bool Battle::loadActionScript(std::string filename) {
  if (filename == "") {
    ...
    std::ifstream in(filename);
    if (!in.is_open()) {
      std::cerr << "[Battle::loadActionScript] File not found: " << filename << std::endl;
      exit(1);
    }
    std::string line;
    bool success = true;
    while (!in.eof() && success) {
      ...
      CharacterVector::iterator pcIt = std::find_if(pcs_.begin(), pcs_.end(),
                                              MatchSource(qa.source->name()));
...
// 1 : find the Character in the list with "name".
...
bool Battle::parseAction(std::string line, QuickAction &qa) {
  ...
  // Initialize both target and source to "error" (pcs_.end() is interpreted as
  // such)
  CharacterVector::iterator it_target = pcs_.end(), it_source = pcs_.end();
  std::shared_ptr<Character> target;
  std::shared_ptr<Character> source;

  // Find the iterator with name "tokens[0]"
  it_source = find_character(tokens[0]);
  // Check to make sure we found an iterator
  if (it_source == pcs_.end()) {
  ...
  if (tokens[2] == "target") {
    ...
    it_target = find_character(tokens[2]);
    // Check to make sure we found an iterator
  ...
  ActionType action_type = N_ACTIONS; // Error code
  if (tokens[1] == "attack")
    action_type = ATTACK;
  else if (tokens[1] == "heal")
    action_type = HEAL;
  else if (tokens[1] == "defend")
    action_type = DEFEND;
  else {
...
bool Battle::performScriptedActions() {
  ...
  if (!scripted_) {
    std::cout << "I am expecting a player script, but you didn't give me one."
              << std::endl;
    return false;
  }
  ...
  if (turn_ == 0 && pcs_.size() > 1) {
    for (CharacterVector::iterator inpc = npcs_.begin(); inpc != npcs_.end();
         ++inpc) {
  ...
  for (CharacterVector::iterator it = npcs_.begin(); it != npcs_.end(); ++it) {
    (*it)->turn_ = turn_;
  }

  for (CharacterVector::iterator it = pcs_.begin(); it != pcs_.end(); ++it) {
    (*it)->turn_ = turn_;
  }
  ...
  for (std::vector<QuickAction>::iterator it = actions_.begin();
    ...
    if (boss != 0) {
      ...
      if (it->action == MULTIATTACK) {
        for (CharacterVector::iterator itarget = pcs_.begin(); itarget != pcs_.end();
             ++itarget) {
    ...
    if (!anyPCAlive()) {
      std::cout << it->source->name() << " has vanquished your party."
                << std::endl;
      return false;
    }

    if (!anyNPCAlive()) {
      std::cout << it->source->name() << " has vanquished your foe!!!"
                << std::endl;
      return false;
    }
  }
...
bool Battle::performUserActions(std::istream &in) {
  ...
  if (scripted_) {
    std::cout << "PC actions already scripted, do not give me scripted actions "
...
bool Battle::setBossTarget(Boss *boss) {
  ...
  Character const *character = boss->getTarget();

  if (boss == 0) {
    std::cout << "Boss is zero, cannot set new target" << std::endl;
    return false;
  }
  for (CharacterVector::const_iterator pc = pcs_.begin(); pc != pcs_.end(); ++pc) {
    if ((*pc)->isAlive()) {
      if (pc->get() != character) {
        std::cout << boss->name() << " shifts their attacks to "
                  << (*pc)->name() << std::endl;
      }
...
bool Battle::anyNPCAlive() const {
  ...
  for (CharacterVector::const_iterator npc = npcs_.begin(); npc != npcs_.end();
       ++npc) {
    if ((*npc)->isAlive())
      any_npc_alive = true;
  }
  return any_npc_alive;
}

Battle::CharacterVector::iterator Battle::find_character(std::string s) {
  // Try to find the sources and targets:
  CharacterVector::iterator source_pc =
      std::find_if(pcs_.begin(), pcs_.end(), MatchSource(s));
  CharacterVector::iterator source_npc =
      std::find_if(npcs_.begin(), npcs_.end(), MatchSource(s));
...
void Battle::printStats(std::ostream &out) {
  ...
  // Remember! "CharacterVector" is just an alias for std::vector<
  // std::shared_ptr<Character> >
  for (CharacterVector::const_iterator it = pcs_.begin(); it != pcs_.end(); ++it) {
    (*it)->printStats(out);
  }
  out << std::endl << "   === monsters:" << std::endl;
  for (CharacterVector::const_iterator it = npcs_.begin(); it != npcs_.end(); ++it) {
    (*it)->printStats(out);
  }
  out << "-----------------------------" << std::endl;
}
...
void Battle::printActions(std::ostream &out) {
  ...
  for (std::vector<QuickAction>::const_iterator it = actions_.begin();
    ...
    out << it->source->name() << " will perform action " << it->action
        << " on their target : ";
