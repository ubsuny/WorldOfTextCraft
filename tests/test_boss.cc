...
int main(int argc, char **argv) {
  ...
  while (!done) {
    ...
    if (i % 5 == 0)
      for (std::vector<Character *>::iterator it = party.begin();
           it != party.end(); ++it) {
        boss.multiAttack(*it);
      }
    else
    ...
    priest.heal();
    ++i;
    done = (warrior.isDead() && priest.isDead() && rogue.isDead()) ||
           boss.isDead();
  }

  if (warrior.isAlive() && priest.isAlive() && rogue.isAlive()) {
    std::cout << "The party wins! The gods rejoice in your ferociousness!"
              << std::endl;
  } else {
    std::cout << "Your quest has failed. May you drink and fight long in the "
                 "mead halls of Valhalla"
              << std::endl;
  }
